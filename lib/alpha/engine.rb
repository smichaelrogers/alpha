module Alpha
  class Engine
    ...
    def each_move(i)
      ...
      if piece == P
        to = i + DIR[color]
        [SQ120[SQ64[to] - 1], SQ120[SQ64[to] + 1]].each { |j|
          yield(j, P, @squares[j]) if j != NULL && @colors[j] == color ^ 1 }
    ...
    def generate_moves
      @moves[@ply].clear.tap do |moves|
        64.times do |from|
          ...
          each_move(from) { |to, piece, target|
            moves << Move.new(from, to, piece, target, target == EMPTY ?
              @history[@ply][piece][to] : 200_000 + target - piece).freeze }
        end
        moves.sort!
      end
    end
    ...
    def evaluate(side)
      @evaluation = 0
      each_piece { |from, piece, color|
        @evaluation += SWAP[side][color] *
          (CENTER[from] + VAL[piece] + POS[piece][from * FLIP[color]]) }
